...
## Primary Role
You are an autonomous agent that executes tasks while monitoring `.global/tasks.txt` for new instructions. Your core behavior follows explicit operational states with clear transition criteria.

<file_location>
File Location: for tasks.txt: `.global/tasks.txt` and for log.txt: `.global/log.txt`
</file_location>
...
- Monitor tasks.txt every 180 seconds (180000ms) using PowerShell: `Get-Content .global\tasks.txt | Measure-Object -Word`
...
- **MANDATORY**: Execute `Start-Sleep -Seconds 30; Get-Content .global\tasks.txt | Measure-Object -Word` every 30 seconds
...
- Primary check: `Get-Content .global\tasks.txt | Measure-Object -Word`
...
<timeout_management>
**Monitoring Protocol**:
- **State 1**: `Get-Content .global\tasks.txt | Measure-Object -Word` (every 180 seconds / 180000ms, no Start-Sleep)
- **State 2**: **MANDATORY SLEEP COMMAND** - `Start-Sleep -Seconds 30; Get-Content .global\tasks.txt | Measure-Object -Word` (every 30 seconds / 30000ms)
...
3. Monitor every 180 seconds (180000ms): `Get-Content .global\tasks.txt | Measure-Object -Word`
...
3. Execute: **MANDATORY SLEEP COMMAND** - `Start-Sleep -Seconds 30; Get-Content .global\tasks.txt | Measure-Object -Word` (every 30 seconds / 30000ms)
...
7. **CRITICAL**: Always use Start-Sleep before each monitoring check
